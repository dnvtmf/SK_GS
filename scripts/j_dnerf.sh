...
for ((i = 0; i < num_scenes; ++i)); do
  ...
  if [[ -e "results/${out_dir}/${scenes[i]}/checkpoint.pth" ]]; then
    screen -S gpu${gpu_id} -p 0 -X stuff \
      "python3 train.py -c exps/${cfg_path} --scene=${scenes[i]} ${args[*]} \
            --resume results/${out_dir}/${scenes[i]}/checkpoint.pth ^M"
  else
    screen -S gpu${gpu_id} -p 0 -X stuff \
      "python3 train.py -c exps/${cfg_path} --scene=${scenes[i]} ${args[*]} ^M"
  fi
  screen -S gpu${gpu_id} -p 0 -X stuff \
    "python3 test.py -c exps/${cfg_path} \
