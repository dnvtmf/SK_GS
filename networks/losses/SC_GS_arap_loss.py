"""
copy from: https://github.com/yihua7/SC-GS/blob/master/utils/deform_utils.py
"""
import numpy as np
...
def cal_connectivity_from_points(
    ...
    # ii : [Ne,] the i th vert
    # jj: [Ne,] j th vert is connected to i th vert.
    ...
    nn_dist[:, least_edge_num:] = torch.where(
        ...
        nn_dist[:, least_edge_num:], torch.ones_like(nn_dist[:, least_edge_num:]) * torch.inf)
    ...
    for idx in range(1, Nt):
        ...
        with torch.no_grad():
            rotation = estimate_rotation(
                nodes_sequence[0], nodes_sequence[idx], ii, jj, nn, K=K, weight=weight, sample_idx=sample_idx
            )  # [Nv, 3, 3]
        # Compute energy
...
def lstsq_with_handles(A, b, handle_idx, handle_pos):
    ...
    handle_mask = torch.zeros_like(A[:, 0], dtype=torch.bool)
...
def arap_deformation_loss(trajectory, node_radius=None, trajectory_rot=None, K=50, with_rot=True):
    ...
    with torch.no_grad():
        ...
        ii, jj, nn, weight = cal_connectivity_from_points(
            init_pcl, radius, K, trajectory=trajectory.detach(), node_radius=node_radius, mode='nn')
    ...
    with torch.no_grad():
        ...
        with torch.no_grad():
            # Need to flip the column of U corresponding to the smallest singular value for any det(Ri) <= 0
            entries_to_flip = torch.nonzero(torch.det(R) <= 0, as_tuple=False).flatten()  # idxs where det(R) <= 0
            if len(entries_to_flip) > 0:
                Umod = U.clone()
                cols_to_flip = torch.argmin(sig[entries_to_flip], dim=1)  # Get minimum singular value for each entry
