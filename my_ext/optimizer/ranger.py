...
# summary of changes:
# full code integration with all updates at param level instead of group,
# moves slow weights into state dict (from generic weights),
...
class Ranger(Optimizer):
    ...
    def __init__(
        self, params, lr=1e-3, alpha=0.5, k=6, N_sma_threshhold=5, betas=(.95, 0.999), eps=1e-5,
        weight_decay=0
    ):
        # parameter checks
        ...
        # This should resolve issues with load/save where weights were left in GPU memory from first load,
        # slowing down future runs.
...
class Ranger2(Optimizer):

    def __init__(
        self, params, lr=1e-3, alpha=0.5, k=6, n_sma_threshhold=5, betas=(.95, 0.999),
