...
__device__ __host__ float distBoxPoint(const MinMax& box, const float3& p) {
  ...
  if (p.y < box.minn.y || p.y > box.maxx.y) diff.y = min(abs(p.y - box.minn.y), abs(p.y - box.maxx.y));
  if (p.z < box.minn.z || p.z > box.maxx.z) diff.z = min(abs(p.z - box.minn.z), abs(p.z - box.maxx.z));
...
void knn(int P, float3* points, float* meanDists) {
  float3* result;
  ...
  coord2Morton KERNEL_ARG((P + 255) / 256, 256)(P, points, minn, maxx, morton.data().get());
  ...
  boxMinMax KERNEL_ARG(num_boxes, BOX_SIZE)(P, points, indices_sorted.data().get(), boxes.data().get());
  boxMeanDist KERNEL_ARG(num_boxes, BOX_SIZE)(P, points, indices_sorted.data().get(), boxes.data().get(), meanDists);

  cudaFree(result);
}

torch::Tensor simple_knn(const torch::Tensor& points) {
  CHECK_CUDA(points);
  CHECK_TYPE(points, torch::kFloat32);
  BCNN_ASSERT(points.ndimension() == 2 && points.size(-1) == 3, "Error shape for points");
  const int P = points.size(0);

  auto float_opts     = points.options().dtype(torch::kFloat32);
  torch::Tensor means = torch::full({P}, 0.0, float_opts);

  knn(P, (float3*) points.contiguous().data<float>(), means.contiguous().data<float>());

  return means;
}
REGIST_PYTORCH_EXTENSION(other_knn, { m.def("simple_knn", &simple_knn); });
}  // namespace SimpleKNN