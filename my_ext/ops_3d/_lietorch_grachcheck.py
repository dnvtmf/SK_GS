# copy from https://github.com/princeton-vl/lietorch/blob/master/lietorch/gradcheck.py
import torch
...
def iter_tensors(x: Union[torch.Tensor, Iterable[torch.Tensor]], only_requiring_grad: bool = False) -> Iterable[
    torch.Tensor]:
    if is_tensor_like(x):
    ...
    if any([o.requires_grad for o in output]):
        ...
        if check_undefined_grad:
            ...
            def check_undefined_grad_support(output_to_check):
                grads_output = [torch.zeros_like(o, memory_format=torch.legacy_contiguous_format) for o in
                                output_to_check]
                try:
