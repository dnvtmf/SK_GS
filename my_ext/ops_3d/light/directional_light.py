...
class DirectionalLight(Light):
    ...
    def __init__(
        self,
        ambient_color: Union[float, Tuple[float, float, float], Tensor] = (0.5, 0.5, 0.5),
        diffuse_color: Union[float, Tuple[float, float, float], Tensor] = (0.3, 0.3, 0.3),
        specular_color: Union[float, Tuple[float, float, float], Tensor] = (0.2, 0.2, 0.2),
        direction: Union[float, Tuple[float, float, float], Tensor] = (0, 1., 0),
        device=None,
        trainable=False
    ) -> None:
        """
        ...
        if isinstance(direction, Tensor):
            ...
            direction = torch.tensor(utils.n_tuple(direction, 3), dtype=torch.float, device=device)
        self.register_buffer('direction', normalize(direction))
        self.direction: Tensor

    def forward(self, points: Tensor = None):
        return self.direction, self.ambient_color, self.diffuse_color, self.specular_color
