...
class VoxelRenderer:
    def __init__(self, render_IO_vox_size=256, render_boundary_padding_size=16):
        ...
        self.voxel_idxs = np.linspace(-self.render_fix_vox_size / 2 + 0.5, self.render_fix_vox_size / 2 - 0.5,
                                      self.render_fix_vox_size, dtype=np.float32)
        self.voxel_x, self.voxel_y, self.voxel_z = np.meshgrid(
            self.voxel_idxs, self.voxel_idxs, self.voxel_idxs, sparse=False, indexing='ij')
    ...
    def render_img(self, voxel_in, threshold, view=0, get_depth=False, ray_x=0, ray_y=0, ray_z=1, steep_threshold=16):
        ...
        voxel[self.render_boundary_padding_size:-self.render_boundary_padding_size,
        self.render_boundary_padding_size:-self.render_boundary_padding_size,
        self.render_boundary_padding_size:-self.render_boundary_padding_size] = \
            (voxel_in > threshold).astype(np.uint8)[::-1, ::-1]
        ...
        if get_depth:  # depth
            ...
            output = output[self.render_boundary_padding_size:-self.render_boundary_padding_size,
                     self.render_boundary_padding_size:-self.render_boundary_padding_size]
            output = np.clip(output, 0, 255).astype(np.uint8)

        return output

    def render_img_with_camera_pose(
            self,
            voxel_in,
            threshold,
            cam_alpha=0.785,
        ...
        if processed:
            ...
            voxel[self.render_boundary_padding_size:-self.render_boundary_padding_size,
            self.render_boundary_padding_size:-self.render_boundary_padding_size,
            self.render_boundary_padding_size:-self.render_boundary_padding_size] = (voxel_in > threshold).astype(
                np.uint8)[::-1, ::-1]
        ...
        if get_depth:  # depth
            ...
            output = output[self.render_boundary_padding_size - 1:-self.render_boundary_padding_size + 1,
                     self.render_boundary_padding_size - 1:-self.render_boundary_padding_size + 1]
            output = np.clip(output, 0, 255).astype(np.uint8)
    ...
    def render_img_with_camera_pose_gpu(
            self,
            voxel_in,
            threshold,
            cam_alpha=0.785,
        ...
        if processed:
            ...
            voxel[self.render_boundary_padding_size:-self.render_boundary_padding_size,
            self.render_boundary_padding_size:-self.render_boundary_padding_size,
            self.render_boundary_padding_size:-self.render_boundary_padding_size] = voxel_in[::-1, ::-1]
        ...
        new_xyz_tensor = torch.cat([new_x_tensor.unsqueeze(3), new_y_tensor.unsqueeze(3), new_z_tensor.unsqueeze(3)],
                                   3).unsqueeze(0)

        voxel_tensor = F.grid_sample(voxel_tensor, new_xyz_tensor, 'bilinear', 'zeros', align_corners=False).squeeze()
        voxel_tensor = voxel_tensor.gt(threshold)
