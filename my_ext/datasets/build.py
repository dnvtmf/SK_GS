...
from my_ext.utils import *
from my_ext.utils.registry import Registry
from my_ext.utils import eval_str, to_list, extend_list
...
def options(DATASETS: Registry, DATASET_STYLE: Registry, parser=None):
    ...
    group.add_argument('--dataset', default=None, type=eval_str,
        help='The datasets use in train/eval/test. '
             ...
             f'Default datastes: {list(DATASETS.keys())}'
    )
    ...
    group.add_argument("--new-datasets", default=None, metavar='E', type=eval_str,
        help=f"Add new datasets. Styles: {list(DATASET_STYLE.keys())}")
    return group


def make(DATASETS: Registry, DATASET_STYLE: Registry, cfg, mode='train', **kwargs):
    ...
    def obtain_datasets(dataset_name: str, configurations: dict):
        ...
        for name in splits:
            ...
            dataset = DATASET_STYLE[style](**parameters)
